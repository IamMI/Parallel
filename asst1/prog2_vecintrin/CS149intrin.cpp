...
void _cs149_vset(__cs149_vec<T> &vecResult, T value, __cs149_mask &mask) {
  // Copy the value to the result vector register if the mask is active, else keep the old value
  // Note: the mask is active if mask.value[i] is true
  for (int i=0; i<VECTOR_WIDTH; i++) {
...
__cs149_vec_float _cs149_vset_float(float value) {
  ...
  __cs149_mask mask = _cs149_init_ones();     // All lanes are active
  _cs149_vset_float(vecResult, value, mask);  // Set all lanes to the same value
  return vecResult;
}
