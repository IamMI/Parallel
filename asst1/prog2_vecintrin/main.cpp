...
void absVector(float* values, float* output, int N) {
  __cs149_vec_float x;                             // input vector
  __cs149_vec_float result;                        // result vector
  __cs149_vec_float zero = _cs149_vset_float(0.f); // zero vector
...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float x;                                    // Crop values
  __cs149_vec_int y;                                      // Crop exponents                     
  __cs149_vec_int zeroInt = _cs149_vset_int(0);   
  __cs149_vec_int oneInt = _cs149_vset_int(1);       
  __cs149_mask maskAll, maskToMul, maskNotToMul, maskToClamp;
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // Load vector of values from values[i: i+VECTOR_WIDTH]
    _cs149_vload_float(x, values+i, maskAll);               
    // Load vector of values from exponents[i: i+VECTOR_WIDTH]
    _cs149_vload_int(y, exponents+i, maskAll);

    // Begin to calculate the result
    _cs149_vgt_int(maskToMul, y, zeroInt, maskAll); // if y > 0
    maskNotToMul = _cs149_mask_not(maskToMul); // if y <= 0


    while(_cs149_cntbits(maskToMul)){
      _cs149_vmult_float(result, result, x, maskToMul);
      _cs149_vsub_int(y, y, oneInt, maskToMul);
      _cs149_vgt_int(maskToMul, y, zeroInt, maskAll);
    }

    // Clamp the result
    __cs149_vec_float max = _cs149_vset_float(9.999999f);
    _cs149_vgt_float(maskToClamp, result, max, maskAll);
    _cs149_vmove_float(result, max, maskToClamp);
